...
- `src/saas/postgres-migrations.ts` includes `0002_saas_tenant_deks`, creating `openclaw_saas.tenant_deks` with forced RLS and adding `dek_version` / `algorithm` columns to `tenant_secrets`.
- `src/saas/tenant-provisioning.ts` can generate and persist an initial encrypted tenant DEK when a KMS provider is supplied; `provisionConfiguredSaasTenant()` wires this to the configured KMS provider.
...
-- ════════════════════════════════════
CREATE TABLE openclaw_saas.tenant_deks (
  ...
  CONSTRAINT pk_tenant_deks PRIMARY KEY (tenant_id, version),
  CONSTRAINT chk_tenant_deks_algorithm CHECK (algorithm IN ('aes-256-gcm'))
);

-- Only one active DEK per tenant
CREATE UNIQUE INDEX idx_saas_tenant_deks_active
  ON openclaw_saas.tenant_deks(tenant_id)
  WHERE is_active = TRUE;

-- Do not use CREATE INDEX CONCURRENTLY inside the migration runner transaction.
CREATE INDEX idx_saas_tenant_deks_tenant_version
...
- `src/saas/envelope-encryption.ts` implements the AES-256-GCM primitive with 12-byte IV, 16-byte auth tag, 32-byte DEK validation, authenticated associated data, and `iv + authTag + ciphertext` packing.
- `src/saas/envelope-encryption.test.ts` covers roundtrip encryption, packed BYTEA roundtrip, wrong key/AAD/tag authentication failure, and invalid input rejection.
- `src/saas/kms.ts` wraps tenant DEKs with the same authenticated encryption primitive for the current local KMS provider.
- Secret read/write integration is intentionally not wired yet; this primitive is the foundation for the later `tenant_secrets` repository.
...
- `src/saas/dek-cache.ts` implements a defensive-copy in-memory DEK cache with TTL expiry, tenant invalidation, and sweep support.
- The cache is not yet wired into tenant secret read/write flows because the `tenant_secrets` repository is still pending.
...
- `deriveSaasLocalKek()` derives a 256-bit local KEK from `OPENCLAW_MASTER_KEY` material via HKDF-SHA256 for self-hosted/local use.
- `src/saas/kms.ts` defines the KMS provider contract and implements `LocalSaasKmsProvider`.
- `resolveConfiguredSaasKmsProvider()` defaults to `OPENCLAW_KMS_PROVIDER=local` and requires `OPENCLAW_MASTER_KEY`; `OPENCLAW_KMS_KEY_ID` defaults to `local:v1`.
- Supported local master key encodings are `hex:`, `base64:`, `base64url:`, or raw UTF-8; decoded key material must be at least 32 bytes.
- External KMS providers (AWS/GCP/Vault) remain planned and are not implemented yet.
...
- `src/saas/tenant-secrets.ts` implements encrypted tenant secret write/read helpers over the SaaS PostgreSQL executor.
- Each operation opens an explicit transaction, sets `app.current_tenant_id` with `SET LOCAL`, resolves the tenant DEK from `tenant_deks`, decrypts it through KMS, and uses AAD-bound AES-256-GCM for the secret payload.
- Write results intentionally return only metadata; read results return plaintext to the caller and do not cache plaintext.
- Audit events, public tenant APIs, deletion/purge, and migration of existing env/auth-profile secrets remain pending.
