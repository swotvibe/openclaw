...
function isOllamaCloudBaseUrl(baseUrl: string | undefined): boolean {
  if (!baseUrl) {
    return false;
  }
  try {
    const parsed = new URL(resolveOllamaApiBase(baseUrl));
...
async function buildOllamaProvider(
  ...
  const allModels = discovered.map((model) =>
    buildOllamaModelDefinition(model.name, model.contextWindow, model.capabilities),
  );

  // When using ollama.com cloud endpoint, /api/tags does not return :cloud models.
  // Append suggested cloud models after verifying cloud auth.
  const isCloud = isOllamaCloudBaseUrl(configuredBaseUrl) || isOllamaCloudBaseUrl(apiBase);
  if (isCloud) {
    const authResult = await checkOllamaCloudAuth(apiBase, opts?.apiKey);
    if (authResult.signedIn) {
      for (const cloudModelId of OLLAMA_SUGGESTED_CLOUD_MODELS) {
        if (!discoveredNames.has(cloudModelId)) {
          const capabilities = resolveOllamaCloudModelCapabilities(cloudModelId);
          allModels.push(buildOllamaModelDefinition(cloudModelId, undefined, capabilities));
        }
      }
    }
  }
...
async function runOllamaDiscovery(ctx: ProviderCatalogContext) {
  ...
  const provider = await buildOllamaProvider(explicit?.baseUrl, {
    ...
    apiKey: explicitApiKey || ollamaKey,
  });
